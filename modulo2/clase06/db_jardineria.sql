-- Remove the database jardineria from the server 
DROP DATABASE IF EXISTS jardineria;
GO

-- create a new databse called db_jardineria
CREATE DATABASE db_jardineria;
go

--use the new created database db_jardineria
--so that all new queries are applied to this database
USE db_jardineria;
go

--create a new table in the database in use called oficina
...
--the same ass previous but this time the
--database is called empleado
...
--select everything from table pago, oficina, empleado, gama_producto, etc.
SELECT * FROM pago;
GO
...
--in this cases only attribute ciudad and telefono
--are selected from table oficina, but also
--in will only select rows in the table that contains
--the attribute pais = "España". This is a filter
...
SELECT ciudad, telefono FROM oficina WHERE pais = "Japón";
GO

--return name, last name and email from worker
--where boss code is equal to 7.
SELECT nombre, apellido1, apellido2, email FROM empleado WHERE codigo_jefer = 7;
GO

SELECT nombre, apellido1, apellido2, email FROM empleado WHERE codigo_jefer IS NULL;
GO
